...
| **Accessibility** | **READY** | **0 axe violations on all seven canonical destinations across the FULL rule set, in both themes** (14/14 route-theme combinations, verified 2026-09-26). Two `heading-order` violations and one `aria-valid-attr-value` (**critical**) on `/schedule` were found only by running the complete rule set — the repository spec checks a subset — and were fixed. The baseline failure was a 2.58:1 avatar. Focus is never removed without a ≥3:1 replacement. Every status carries a non-colour glyph that survives `prefers-reduced-motion`. Route change moves focus to `main`. Quick Capture has a real label. |
