...
(async () => {
  ...
  await check("Path O: capabilities stay honest about unconfigured engines", async () => {
    ...
    assert(hyper.verified === false || hyper.status === "unconfigured" || hyper.status === "architecturally-ready" || hyper.status === "blocked", hyper.status);
    assert(typeof hyper.processing_ready === "boolean", "hyperframes processing_ready missing");
    assert(hyper.processing_ready === false, "hyperframes cloud must not claim processing_ready");
    assert(typeof openshorts.processing_ready === "boolean", "openshorts processing_ready missing");
    if (!openshorts.configured) assert(openshorts.processing_ready === false, "unconfigured openshorts cannot be processing_ready");
